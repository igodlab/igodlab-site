...
import { FullSlug, isRelativeURL, pathToRoot, resolveRelative } from "../../util/path"
    ...
    handlers: {
      ...
      a(h, node) {
        if (node.properties.dataFootnoteRef === "") {
          const identifier = (node.properties?.id as string).replace("user-content-fnref-", "")
      ...
      p(h, node) {
        if (node.properties.dataCodeblock === "sms" || node.properties.dataCodeblock === "quotes") {
  ...
  return {
    ...
    async *emit(ctx, content, resources) {
      ...
      for (const [tree, file] of content) {
        yield llmText(ctx, tree, file.data, allFiles, resources, reconstructed)
      }
      yield write({
        ctx,
